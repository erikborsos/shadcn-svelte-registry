...
{#if items.length}
	<div class="grid gap-4 sm:grid-cols-2">
		{#each items as item (item.name)}
			...
			>
			<div
				class="pointer-events-none flex min-h-36 flex-1 items-center justify-center overflow-hidden rounded-lg border bg-background bg-[radial-gradient(var(--color-border)_1px,transparent_1px)] bg-size-[12px_12px] p-4"
			>
				{#if item.demo}
					<item.demo />
				{:else}
